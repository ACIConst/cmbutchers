...
// Item Modal
// ═══════════════════════════════════════════════════════════════════════════════
function ItemModal({item,isNew,saving,catNames,onChange,onSave,onClose,menu}){const{T:C,TF:F}=useAdminTheme();
  ...
  return(<Modal t={C} title={isNew?"Add Menu Item":"Edit Menu Item"} onClose={onClose} wide>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}><Field t={C} label="Item Name *"><input value={item.name} onChange={e=>onChange({name:e.target.value})} placeholder="e.g. Ribeye Steak" style={inputSt(false,C)}/></Field><Field t={C} label="Price ($) *"><input value={item.price} onChange={e=>onChange({price:e.target.value})} placeholder="0.00" type="number" step="0.01" style={inputSt(false,C)}/></Field></div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,marginBottom:14}}><Field t={C} label="Category"><select value={item.category} onChange={e=>onChange({category:e.target.value})} style={inputSt(true,C)}>{(catNames||[]).map(c=><option key={c} value={c}>{c}</option>)}</select></Field><Field t={C} label="SKU"><input value={item.sku||""} onChange={e=>onChange({sku:e.target.value})} placeholder="e.g. RIB-001, GB-5LB" style={inputSt(false,C)}/></Field><Field t={C} label="Stock Qty"><input value={item.stock===null||item.stock===undefined?"":item.stock} onChange={e=>onChange({stock:e.target.value})} placeholder="Leave blank = unlimited" type="number" min="0" style={inputSt(false,C)}/></Field></div>
