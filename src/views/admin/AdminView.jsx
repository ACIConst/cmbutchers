...
function ItemModal({item,isNew,saving,catNames,onChange,onSave,onClose,menu}){const{T:C,TF:F}=useAdminTheme();const isQB=!!item.qbItemId;
  ...
  return(<Modal t={C} title={isNew?"Add Menu Item":"Edit Menu Item"} onClose={onClose} wide>
    ...
    {isQB?(item.image?<Field t={C} label="Product Photo (from QuickBooks)" style={{marginBottom:14}}><div style={{marginTop:6}}><Img src={item.image} alt="preview" style={{height:110,width:"100%",objectFit:"cover",borderRadius:10,border:"1px solid "+C.border}}/></div></Field>:<div style={{fontSize:12,color:C.muted,marginBottom:14,padding:"10px 14px",background:C.surface,borderRadius:10,border:"1px solid "+C.border}}>No photo found in QuickBooks. Add one in QB and re-sync.</div>):<><Field t={C} label="Product Photo" style={{marginBottom:14}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>{filtered.map(order=>{const isExpanded=expanded===order.id;const checkedItems=order.checkedItems||[];const totalItems=order.items?.length||0;const allChecked=totalItems>0&&checkedItems.length===totalItems;const curStatus=getStatus(order);const orderCancelled=curStatus==="cancelled";const nextStatuses=ORDER_STATUSES.filter(s=>canTransition(curStatus,s.id));return(
          <div key={order.id} style={{background:orderCancelled?"rgba(69,10,10,.15)":C.card,border:"2px solid "+(orderCancelled?C.red+"44":isExpanded?C.borderMid:C.border),borderRadius:12,overflow:"hidden",transition:"border .2s",opacity:orderCancelled?.75:1}}>
            <div style={{padding:"12px 16px"}}><div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
              ...
              {!order.archived&&!orderCancelled&&<div style={{display:"flex",gap:4,flexShrink:0,alignItems:"center",flexWrap:"wrap"}}><StatusBadge status={curStatus}/>{(curStatus==="placed"||curStatus==="paid")&&<button disabled={savingStatus===order.id} onClick={()=>printAndPick(order)} style={{background:"#1e3a5f",color:"#93c5fd",border:"1px solid #3b82f6",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontFamily:F.body,fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>Pick Ticket</button>}{order.qbInvoiceId&&!order.qbInvoiceSent&&<button disabled={sendingInvoice===order.id} onClick={()=>handleSendInvoice(order)} style={{background:"#1a3a2a",color:"#7ee8a8",border:"1px solid #2CA01C",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontFamily:F.body,fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>{sendingInvoice===order.id?"Sending...":"Send Invoice"}</button>}{order.qbInvoiceSent&&<span style={{background:"#0b3d1a",color:"#4ade80",borderRadius:6,padding:"3px 8px",fontSize:10,fontWeight:700}}>Invoice Sent</span>}{curStatus==="picking"&&<button disabled={savingStatus===order.id} onClick={()=>setStatus(order.id,"out_for_delivery")} style={{background:C.surface,color:"#67e8f9",border:"1px solid #0e7490",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontFamily:F.body,fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>Send to Delivery</button>}{nextStatuses.filter(s=>!(curStatus==="paid"&&s.id==="picking")&&!(curStatus==="picking"&&s.id==="out_for_delivery")&&s.id!=="delivered"&&s.id!=="paid").map(s=><button key={s.id} disabled={savingStatus===order.id} onClick={()=>setStatus(order.id,s.id)} style={{background:C.surface,color:s.text,border:"1px solid "+s.color,borderRadius:6,padding:"4px 10px",cursor:"pointer",fontFamily:F.body,fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>{s.label}</button>)}{curStatus==="placed"&&<button disabled={savingStatus===order.id} onClick={()=>setStatus(order.id,"paid")} style={{background:order.qbPaid?"#166534":"#7f1d1d",color:order.qbPaid?"#4ade80":"#fca5a5",border:"1px solid "+(order.qbPaid?"#22c55e":"#dc2626"),borderRadius:6,padding:"4px 10px",cursor:"pointer",fontFamily:F.body,fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>{order.qbPaid?"Paid":"Unpaid"}</button>}{curStatus!==terminalStatus&&<button disabled={savingStatus===order.id} onClick={()=>completeOrder(order.id)} style={{background:"#166534",color:"#4ade80",border:"1px solid #22c55e",borderRadius:6,padding:"4px 12px",cursor:"pointer",fontFamily:F.body,fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>{savingStatus===order.id?"...":"Delivered"}</button>}{curStatus!==terminalStatus&&<button onClick={()=>setConfirmCancel(order.id)} style={{background:C.errorBg,color:C.errorText,border:"1px solid "+C.errorText,borderRadius:6,padding:"4px 8px",cursor:"pointer",fontSize:14,lineHeight:1,fontWeight:700}} title="Cancel order">{"\u{1F5D1}"}</button>}</div>}
