          ...
          <div key={order.id} style={{background:orderCancelled?"rgba(69,10,10,.15)":C.card,border:"2px solid "+(orderCancelled?C.red+"44":isExpanded?C.borderMid:C.border),borderRadius:12,overflow:"hidden",transition:"border .2s",opacity:orderCancelled?.75:1}}>
            <div style={{padding:"12px 16px"}}><div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
              ...
              {!order.archived&&!orderCancelled&&<div style={{display:"flex",gap:4,flexShrink:0,alignItems:"center",flexWrap:"wrap"}}>
                ...
                <button disabled={savingStatus===order.id} onClick={()=>printAndPick(order)} style={{background:"#1e3a5f",color:"#93c5fd",border:"1px solid #3b82f6",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontFamily:F.body,fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>Pick Ticket</button>
                {curStatus==="paid"&&<button disabled={savingStatus===order.id} onClick={()=>setStatus(order.id,"out_for_delivery")} style={{background:C.surface,color:"#67e8f9",border:"1px solid #0e7490",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontFamily:F.body,fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>Send to Delivery</button>}
