...
function OrderHistory({ orders, users, dbOps, showToast }) {
  const [search,setSearch]=useState("");const [expanded,setExpanded]=useState(null);const [confirmClear,setConfirmClear]=useState(false);const [view,setView]=useState("active");const [statusFilter,setStatusFilter]=useState("all");const [savingStatus,setSavingStatus]=useState(null);const [customerFilter,setCustomerFilter]=useState("all");const [scanBarcode,setScanBarcode]=useState(null);const [confirmDelete,setConfirmDelete]=useState(null);
  ...
  async function handleClearAll(){try{await dbOps.clearOrders();showToast("Orders archived");setConfirmClear(false);setView("active");}catch(e){console.error(e);showToast("Failed","error");}}   async function handleDeleteOrder(id){setSavingStatus(id);try{await dbOps.deleteOrder(id);showToast("Order deleted");setConfirmDelete(null);if(expanded===id)setExpanded(null);}catch(e){console.error(e);showToast("Delete failed","error");}finally{setSavingStatus(null);}}
  return(
    <div style={{animation:"fadeUp .3s ease"}}>
      ...
      {confirmClear&&<ConfirmModal message={"Archive all "+active.length+" active order"+(active.length!==1?"s":"")+"?"} confirmLabel="Archive All" onConfirm={handleClearAll} onClose={()=>setConfirmClear(false)}/>}       {confirmDelete&&<ConfirmModal message={"Are you sure you want to cancel this order? This cannot be undone."} confirmLabel="Cancel Order" danger onConfirm={()=>handleDeleteOrder(confirmDelete)} onClose={()=>setConfirmDelete(null)}/>}
    </div>
  );
}
