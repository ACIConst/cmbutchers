...
// Categories Manager
// ═══════════════════════════════════════════════════════════════════════════════
function CategoriesManager({ categories, menu, dbOps, showToast, isMobile }) {const{T:C,TF:F}=useAdminTheme();
          ...
          <div key={order.id} style={{background:orderCancelled?"rgba(69,10,10,.15)":C.card,border:"2px solid "+(orderCancelled?C.red+"44":isExpanded?C.borderMid:C.border),borderRadius:12,overflow:"hidden",transition:"border .2s",opacity:orderCancelled?.75:1}}>
            <div style={{padding:"12px 16px"}}><div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
              ...
              {!order.archived&&!orderCancelled&&<div style={{display:"flex",gap:4,flexShrink:0,alignItems:"center",flexWrap:"wrap"}}>
                {curStatus==="picking"?<span onClick={()=>printAndPick(order)} style={{cursor:"pointer"}} title="Click to reprint pick ticket"><StatusBadge status={curStatus}/></span>:<StatusBadge status={curStatus}/>}
                {curStatus==="placed"&&order.qbInvoiceId&&!order.qbInvoiceSent&&<button disabled={sendingInvoice===order.id} onClick={()=>handleSendInvoice(order)} style={{background:"#1a3a2a",color:"#7ee8a8",border:"1px solid #2CA01C",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontFamily:F.body,fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>{sendingInvoice===order.id?"Sending...":"Send Invoice"}</button>}
                ...
                {(curStatus==="paid"||curStatus==="picking")&&<button disabled={savingStatus===order.id} onClick={()=>setStatus(order.id,"out_for_delivery")} style={{background:"#0e4a5c",color:"#67e8f9",border:"1px solid #22d3ee",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontFamily:F.body,fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>Send to Delivery</button>}
